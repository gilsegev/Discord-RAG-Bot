...
runner.nodes.push(
  ...
  http('Apply Replacement','http://127.0.0.1:5678/webhook/rag-incremental-phase-9c4',"={{ { action:'apply', incremental_run_id:$items('Normalize Run Request')[0].json.incremental_run_id, runtime_revision:Number(($json.body||$json).runtime_revision), confirm_apply:'PHASE9C4_APPLY', take_full_snapshot:$items('Verify Ready Attempt')[0].json.report?.phase === '9C.6' } }}"),
